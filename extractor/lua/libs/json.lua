...
local function object_or_array(self, T, etc, forceTable)
   --
   ...
   local string_keys = { }
   local number_keys = { }
   local number_keys_must_be_strings = forceTable or false
   ...
   if not number_keys_must_be_strings and T[1] == nil and T[2] == nil then
		number_keys_must_be_strings = true;
   end
...
local function encode_value(self, value, parents, etc, options, indent, for_key)
	local isTopLevelTable = options.isTopLevelTable;
	options.isTopLevelTable = false;
   --
   ...
   elseif type(value) ~= 'table' then
	  ...
	  if type(value) == 'function' then
		___fn_count = ___fn_count + 1;
		return "\"---FUNCTION " .. ___fn_count .. "---\""
	  end
   ...
   elseif getmetatable(value) == isNumber then
      ...
      local object_keys, maximum_number_key, map = object_or_array(self, T, etc, isTopLevelTable)
...
function OBJDEF:encode(value, forceFirstTable, etc, options)
	if type(self) ~= 'table' or self.__index ~= OBJDEF then
   ...
   options.isTopLevelTable = forceFirstTable;
