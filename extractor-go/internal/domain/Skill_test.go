...
func buildDummySkill() domain.Skill {
	return domain.Skill{
		...
		SpCost:         []int32{1, 2, 3},
		CanSelectLevel: domain.NewNullableBool(true),
...
func TestSkill_Equals(t *testing.T) {
	...
	// Test SpAmount
	toSkill = buildDummySkill()
	toSkill.SpCost = []int32{1, 2, 4}
	assert.False(t, fromSkill.Equals(toSkill))

	// Test SeparateLevel
	toSkill = buildDummySkill()
	toSkill.CanSelectLevel = domain.NewNullableBool(false)
	assert.False(t, fromSkill.Equals(toSkill))
	...
	// Test NeedSkillList
	toSkill = buildDummySkill()
	toSkill.RequiredSkills = []domain.NeedSkillEntry{
		{SkillID: 2, Level: 2},
	}
	assert.False(t, fromSkill.Equals(toSkill))
}
