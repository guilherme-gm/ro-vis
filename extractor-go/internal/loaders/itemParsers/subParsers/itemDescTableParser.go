...
func ParseItemDescTable(filePath string) ([]ItemDescTableEntry, error) {
	itemDescriptions := []ItemDescTableEntry{}
	...
	for idx < len(lines) {
		...
		itemDescriptions = append(itemDescriptions, ItemDescTableEntry{
			ItemID:      int32(itemID),
			Description: desc,
		})
	}
